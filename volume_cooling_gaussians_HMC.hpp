                     ...
                     RandomNumberGenerator& rng)
{
    ...
    std::cout << "Sampling of N points from the gaussian distribution: " << last_a <<" "<< duration.count() << std::endl;
    while (!done)
    {
                                ...
                                RandomNumberGenerator& rng)
{
    ...
    while (true)
    {
        ...
        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
        if (duration.count() != (long)0) {
            std::cout << "testing for existance of next gaussian 1: " << a_vals[it] <<" "<<duration.count() << std::endl;
        }
        ...
        for (unsigned  int j = 0; j < steps; j++)
        {
            ...
            if (duration.count() != (long)0) {
                std::cout << "testing for existance of next gaussian 2: " << a_vals[it] << " "<<duration.count() << std::endl;
             }
            curr_its += 1.0;
                                ...
                                unsigned int const& walk_length = 1)
{
         ...
         fnIt++, itsIt++, avalsIt++, i++)
    {
        ...
        update_delta<WalkType>
                ::apply(walk, std::sqrt(std::max(NT(1.0), *avalsIt) * NT(n)) * M_PI
                         / 10);
        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
        if (duration.count() != (long)0) {
            std::cout << "convergence test 1: " << *avalsIt <<" " <<duration.count() << std::endl;
        }
        start = std::chrono::high_resolution_clock::now();
        ...
        duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
        if (duration.count() != (long)0) {
                std::cout << "convergence test 2: " << *avalsIt <<" " <<duration.count() << std::endl;
        }
#ifdef VOLESTI_DEBUG
