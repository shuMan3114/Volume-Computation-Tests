                      ...
                      RandomNumberGenerator &rng)
    {
        ...
        for (auto j=0u; j<walk_length; ++j)
        {
            ...
            while (it < _rho)
            {
                ...
                std::cout << "Updated position p and v for a vpolytope/hpolytope:\n " << _p << "\n _v: " << _v <<std::endl;
                P.compute_reflection(_v, _p, pbpair.second);
                std::cout << "Returning the reflected vector: " << _v << std::endl;
                it++;
            }
            if (it == _rho){
                _p = p0;
        ...
        std::cout << "The given T is: " << T << " It took " << it << " To get the point when omega was: " << _omega << " And the _Len attribute had value " << _Len << " The passed walk_length is: " << walk_length << std::endl;
        p = _p;
    }
